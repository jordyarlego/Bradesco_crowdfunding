...
import SidebarTomador from '../../../app/components/SidebarTomador';
...
export default function EmprestimoAtivo() {
  ...
  const { id } = params;
  const emprestimoId = id?.split('/')?.pop();
  ...
  }, [loading, autenticado, usuario, id]);
  ...
  const carregarDados = async () => {
    ...
    try {
      ...
      const emp = await emprestimoService.buscarEmprestimoPorId(emprestimoId, token);
      setEmprestimo(emp);

      const pags = await pagamentoService.listarPagamentosPorEmprestimo(emprestimoId, token);
        ...
        <main className="flex-1 p-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            <div className="bg-gradient-to-br from-brand-purple-light to-brand-purple-dark rounded-2xl p-6 text-white shadow-xl">
              ...
              <p className="text-3xl font-bold">{formatarMoeda(emprestimo.montanteInicial)}</p>
            </div>
            ...
            <div className="bg-gradient-to-br from-green-500 to-green-600 rounded-2xl p-6 text-white shadow-xl">
              ...
              <p className="text-3xl font-bold">{emprestimo.taxaJuros}% a.m.</p>
            </div>
          </div>

          {/* Dados gerais */}
          <div className="bg-white rounded-2xl p-6 shadow-sm mb-8">
            <h2 className="text-xl font-bold text-brand-purple-dark mb-6">Informações do Empréstimo</h2>
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm">
            <div className="flex justify-between mb-6">
              <h2 className="text-xl font-bold text-brand-purple-dark">Parcelas</h2>

              <div className="flex gap-2 flex-wrap">
                {['todas', 'paga', 'em_aberto', 'atrasada', 'futura'].map((st) => (
                  <button
                    key={st}
                    ...
                    <div className="flex items-center gap-4 flex-1">
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-3 mb-1">
                          <p className="font-semibold">Parcela {p.numero}</p>
                          <span className={`px-3 py-1 rounded-full text-xs font-semibold ${getStatusBadge(p.status)}`}>
                            {getStatusLabel(p.status)}
                          </span>
                        </div>

                        <div className="flex gap-4 text-sm text-gray-600">
                          <span><strong>Valor:</strong> {formatarMoeda(p.valor)}</span>
                          <span><strong>Vencimento:</strong> {p.vencimento}</span>
                        </div>
                      </div>
                    </div>
